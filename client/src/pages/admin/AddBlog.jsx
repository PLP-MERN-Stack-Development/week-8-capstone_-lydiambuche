...
function AddBlog() {
  ...
  const generateContent = async () => {
    ...
    try {
      setLoading(true);
      const { data } = await axios.post('/api/blog/generate', {prompt: title}, {headers: {'authorization': token}});
      if (data.success) {
        quillRef.current.root.innerHTML = parse(data.content);
      } else {
        toast.error(data.message + 'h2');
      }
    } catch (error) {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post('/api/blog/add', formData, {headers: {'authorization': token}});
    ...
    <form onSubmit={onSubmitHandler} className='flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll'>
      <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded'>
        ...
        <div className='max-w-lg h-74 pb-16 sm:pb-10 pt-2 relative'>
          ...
          {loading && (
            <div className='absolute right-0 top-0 bottom-0 left-0 flex items-center justify-center bg-blavk/10 mt-2'>
