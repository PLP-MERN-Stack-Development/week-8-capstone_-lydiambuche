...
const BlogTableItem = ({blog, fetchBlog, index}) => {
  ...
  const deleteBlog = async () => {
    ...
    try {
      ...
      const { data } = await axios.post('/api/blog/delete', {id: blog._id}, {headers: {'authorization': token}});
      if (data.success) {
  ...
  const togglePublish = async () => {
    try {
      ...
      const { data } = await axios.post('/api/blog/toggle-publish', {id: blog._id}, {headers: {'authorization': token}});
      if (data.success) {
        toast.success(data.message);
        await fetchBlog();
